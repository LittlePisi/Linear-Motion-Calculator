...
const InputParameters: React.FC<InputParametersProps> = ({ params, setParams, darkMode }) => {
  ...
  const commonParameters = [
    { label: 'Рабочий ход', key: 'stroke', unit: 'мм', step: 10, min: 0, max: 1000},
  ...
  return (
    <div className={`rounded-xl shadow-lg p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      ...
      <div className="space-y-3">
        ...
        {(params.useMaxSpeedMode ? maxSpeedParameters : travelTimeParameters).map(({ label, key, unit, step, min, max }) => (
        ...
        {/* Common parameters */}
        {commonParameters.map(({ label, key, unit, step, min, max }) => (
          <div key={key} className="flex items-center gap-2">
            <label className={`text-sm w-1/3 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {label}
            </label>
